...
def main():
    ...
    for total_batch in range(TOTAL_BATCH):
        ...
        generate_samples(sess, generator, BATCH_SIZE, generated_num, negative_file)
        dis_eval_loader = Dis_dataloader(BATCH_SIZE)
        dis_eval_loader.load_train_data(positive_file, negative_file)
        
        # Evaluation metrics
        pre_update_metrics = evaluate_discriminator(sess, discriminator, dis_eval_loader, dis_dropout_keep_prob)
        ##======## NEW ##======##    
        ...
        post_update_metrics = evaluate_discriminator(sess, discriminator, dis_eval_loader, dis_dropout_keep_prob)
        
        # Log both metrics
        metrics_log.write('adv-train-epoch\t{0}\t{1:.4f}\t{2:.4f}\t{3:.4f}\t{4:.4f}\t{5:.4f}\t{6:.4f}\t{7:.4f}\t{8:.4f}\t{9:.4f}\n'.format(
            total_batch, 
            avg_g_loss,
            pre_update_metrics['avg_d_loss'], 
            pre_update_metrics['avg_real_prob'], 
            pre_update_metrics['avg_fake_prob'],
            pre_update_metrics['overall_accuracy'],
            post_update_metrics['avg_d_loss'],
            post_update_metrics['avg_real_prob'],
            post_update_metrics['avg_fake_prob'],
            post_update_metrics['overall_accuracy']))  # Now this will be included in the output
        metrics_log.flush()
        ##======## NEW ##======##
...
def evaluate_discriminator(sess, discriminator, data_loader, dropout_keep_prob):
    """Evaluate discriminator on a dataset without updating weights."""
    data_loader.reset_pointer()
    d_loss_sum = 0.0
    real_probs_sum = 0.0
    fake_probs_sum = 0.0
    ...
    for it in xrange(data_loader.num_batch):
        ...
        d_loss, d_probs = sess.run([discriminator.loss, discriminator.ypred_for_auc], feed)
...
def load_metrics_file(file_path):
    """Function to parse the metrics file with pre and post update metrics"""
    # Load the metrics file
    df = pd.read_csv(file_path, sep='\t')
    
    # Convert numeric columns - now with pre/post metrics
    numeric_cols = ['g_loss', 'pre_d_loss', 'pre_real_prob', 'pre_fake_prob', 'pre_accuracy',
                    'post_d_loss', 'post_real_prob', 'post_fake_prob', 'post_accuracy']
    for col in numeric_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')
    
    # Split by phase
    pre_train_d = df[df['phase'] == 'pre-train-d-epoch'].copy()
    adv_train = df[df['phase'] == 'adv-train-epoch'].copy()
    
    # For pre-training data, we need to handle the column alignment issue
    # The 'pre_d_loss' column contains '-' and actual loss is in the next column
    if not pre_train_d.empty:
        # The actual loss values are in 'pre_real_prob' column for pre-training
        pre_train_d.loc[:, 'd_loss'] = pre_train_d['pre_real_prob']
        # Shift columns right for actual values
        pre_train_d['avg_real_prob'] = pre_train_d['pre_fake_prob']
        pre_train_d['avg_fake_prob'] = pre_train_d['pre_accuracy']
        pre_train_d['overall_accuracy'] = pre_train_d['post_d_loss']
    
    return pre_train_d, adv_train

def parse_log_file(file_path):
    """Parse the experiment log file to extract NLL values"""
    pre_train_epochs = []
    pre_train_nlls = []
    adv_train_epochs = []
    adv_train_nlls = []
...
def visualize_seqgan_results(log_file_path='save/experiment-log.txt', metrics_file_path='save/training-metrics.txt'):
    ...
    # Create a multi-panel figure with 2 rows and 3 columns
    ...
    if adv_train_epochs and adv_train_nlls:
        ...
        ax1.plot(adv_train_epochs_adjusted, adv_train_nlls, 'r-', marker='o', markersize=6, label='SeqGAN')
    ...
    if has_metrics:
        # 2. Plot Generator Loss (top right panel)
        ax2 = fig.add_subplot(2, 3, 2)
        
        # For adversarial training phase only (since generator loss is only available during this phase)
        if not adv_train.empty:
            adv_train_grouped = adv_train.groupby('epoch').mean(numeric_only=True)
            # Use the same epoch adjustment as in NLL plot for consistency
            if pre_train_epochs:
                last_pre_train_epoch = pre_train_epochs[-1]
                adjusted_epochs = [last_pre_train_epoch + e + 1 for e in adv_train_grouped.index]
            else:
                adjusted_epochs = adv_train_grouped.index
                
            ax2.plot(adjusted_epochs, adv_train_grouped['g_loss'], 
                     'r-', marker='o', markersize=6, label='Generator Loss')
        
        ax2.set_xlabel('Training Epochs', fontsize=12)
        ax2.set_ylabel('Generator Loss', fontsize=12)
        ax2.set_title('Generator Training Loss (Adversarial Phase)', fontsize=14)
        ax2.grid(True, linestyle='--', alpha=0.7)
        ax2.legend(fontsize=10)
        
        # 3. Plot Discriminator Loss Before and After Update
        ax3 = fig.add_subplot(2, 3, 3)
        
        if not pre_train_d.empty and not adv_train.empty:
            ...
            # Get last pre-train epoch for offset
            pre_train_max_epoch = pre_train_d_grouped.index.max()
            
            # Adjust adversarial training epochs to continue after pre-training
            adjusted_adv_epochs = [pre_train_max_epoch + e + 1 for e in adv_train_grouped.index]
            
            # For pre-training, use the corrected d_loss column
            if 'd_loss' in pre_train_d_grouped.columns:
                # Plot pre-training discriminator loss
                ax3.plot(pre_train_d_grouped.index, pre_train_d_grouped['d_loss'], 
                        'b-', marker='o', markersize=4, label='Pre-training D Loss')
            
            # For adversarial training, plot both pre and post update losses
            if 'pre_d_loss' in adv_train_grouped.columns and 'post_d_loss' in adv_train_grouped.columns:
                ax3.plot(adjusted_adv_epochs, adv_train_grouped['pre_d_loss'], 
                        'r-', linewidth=6, label='Pre-update D Loss')
                ax3.plot(adjusted_adv_epochs, adv_train_grouped['post_d_loss'], 
                        'g-', linewidth=3, label='Post-update D Loss')
        
        ax3.set_xlabel('Training Epochs', fontsize=12)
        ax3.set_ylabel('Discriminator Loss', fontsize=12)
        ax3.set_title('Discriminator Training Loss', fontsize=14)
        ax3.grid(True, linestyle='--', alpha=0.7)
        ax3.legend(fontsize=10)
        
        # 4. Plot Discriminator Pre-Update Probabilities (bottom left)
        ax4 = fig.add_subplot(2, 3, 4)
        
        if not pre_train_d.empty and not adv_train.empty:
            # Group by epoch and get means
            pre_train_d_grouped = pre_train_d.groupby('epoch').mean(numeric_only=True)
            adv_train_grouped = adv_train.groupby('epoch').mean(numeric_only=True)
            ...
            # Plot pre-training probabilities (using corrected column names)
            if 'avg_real_prob' in pre_train_d_grouped.columns and 'avg_fake_prob' in pre_train_d_grouped.columns:
                ax4.plot(pre_train_d_grouped.index, pre_train_d_grouped['avg_real_prob'], 
                        'b-', marker='o', markersize=4, label='Pre-train Real Prob')
                ax4.plot(pre_train_d_grouped.index, pre_train_d_grouped['avg_fake_prob'], 
                        'b--', marker='x', markersize=4, label='Pre-train Fake Prob')
            
            # Plot adversarial training pre-update probabilities
            if 'pre_real_prob' in adv_train_grouped.columns and 'pre_fake_prob' in adv_train_grouped.columns:
                ax4.plot(adjusted_adv_epochs, adv_train_grouped['pre_real_prob'], 
                        'r-', marker='o', markersize=4, label='Pre-update Real Prob')
                ax4.plot(adjusted_adv_epochs, adv_train_grouped['pre_fake_prob'], 
                        'r--', marker='x', markersize=4, label='Pre-update Fake Prob')
        ...
        ax4.set_title('Discriminator Pre-Update Probabilities', fontsize=14)
        ax4.set_ylim(-0.2, 1.2)
        ax4.grid(True, linestyle='--', alpha=0.7)
        ax4.legend(fontsize=9, loc='center right')
        
        # 5. Plot Discriminator Post-Update Probabilities (bottom middle)
        ...
        if not adv_train.empty:
            ...
            # Get last pre-train epoch for offset
            if not pre_train_d.empty:
                pre_train_max_epoch = pre_train_d.groupby('epoch').max().index.max()
            else:
                pre_train_max_epoch = 0
                
            # Adjust adversarial training epochs to continue after pre-training
            adjusted_adv_epochs = [pre_train_max_epoch + e + 1 for e in adv_train_grouped.index]
            
            # Plot adversarial training post-update probabilities
            if 'post_real_prob' in adv_train_grouped.columns and 'post_fake_prob' in adv_train_grouped.columns:
                ax5.plot(adjusted_adv_epochs, adv_train_grouped['post_real_prob'], 
                        'g-', marker='o', markersize=4, label='Post-update Real Prob')
                ax5.plot(adjusted_adv_epochs, adv_train_grouped['post_fake_prob'], 
                        'g--', marker='x', markersize=4, label='Post-update Fake Prob')
        
        # Add the ideal 0.5 line
        ax5.axhline(y=0.5, color='g', linestyle='--', label='Ideal GAN (0.5)')
        
        ax5.set_xlabel('Training Epochs', fontsize=12)
        ax5.set_ylabel('Probability', fontsize=12)
        ax5.set_title('Discriminator Post-Update Probabilities', fontsize=14)
        ax5.set_ylim(-0.2, 1.2)
        ax5.grid(True, linestyle='--', alpha=0.7)
        ax5.legend(fontsize=9, loc='center right')
        
        # 6. Plot Discriminator Accuracy (bottom right)
        ...
        if not adv_train.empty:
            ...
            # Only plot pre_accuracy since post_accuracy is missing from the data
            if 'pre_accuracy' in adv_train_grouped.columns:
                ax6.plot(adjusted_adv_epochs, adv_train_grouped['pre_accuracy'], 
                        'r-', linewidth=6, label='Pre-update Accuracy')
            
            # Calculate the post-accuracy ourselves if we have real and fake probabilities
            if 'post_real_prob' in adv_train_grouped.columns and 'post_fake_prob' in adv_train_grouped.columns:
                # A simple way to compute accuracy from probabilities (correctly classifying real as 1 and fake as 0)
                # This is an approximation - might differ from the original intended calculation
                post_accuracy = (adv_train_grouped['post_real_prob'] + (1 - adv_train_grouped['post_fake_prob'])) / 2
                ax6.plot(adjusted_adv_epochs, post_accuracy, 
                        'g-', linewidth=3, label='Post-update Accuracy (estimated)')
            
            # Also plot overall_accuracy from pre-training for completeness
            if 'overall_accuracy' in pre_train_d_grouped.columns:
                ax6.plot(pre_train_d_grouped.index, pre_train_d_grouped['overall_accuracy'],
                        'b-', marker='o', markersize=4, label='Pre-training Accuracy')

        # Add the ideal 0.5 line
        ax6.axhline(y=0.5, color='b', linestyle='--', label='Random Classifier (0.5)')

        ax6.set_xlabel('Training Epochs', fontsize=12)
        ax6.set_ylabel('Accuracy', fontsize=12)
        ax6.set_title('Discriminator Accuracy', fontsize=14)
        ax6.set_ylim(0.4, 1.1)  # Set appropriate range for accuracy values
...
visualize_seqgan_results('save/experiment-log.txt', 'save/training-metrics.txt')